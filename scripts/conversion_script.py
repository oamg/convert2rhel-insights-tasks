...
# Path to the convert2rhel migration results file.
C2R_MIGRATION_RESULTS_FILE = "/etc/migration-results"
...
def generate_report_message(migration_result, highest_status):
    """Generate a report message based on the status severity."""
    message = ""
    if STATUS_CODE[highest_status] <= STATUS_CODE["WARNING"] and migration_result:
...
def parse_migration_results():
    """Method that will parse the migration results"""
    if not os.path.exists(C2R_MIGRATION_RESULTS_FILE):
        return False
...
def main():
    ...
    try:
        ...
        migration_result = parse_migration_results()
