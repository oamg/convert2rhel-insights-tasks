...
def get_rollback_failures(returncode):
    ...
    try:
        with open(C2R_LOG_FILE, mode="r") as handler:
            ...
            # If the end message was found, use the first occurrence of it.
            # If wasn't found, use the rest of the log
            end_index = end_message_occurrences[0] if end_message_occurrences else None
...
def gather_json_report(report_file, returncode):
    """Collect the json report generated by convert2rhel."""
    logger.info("Collecting JSON report.")

    # Do not gather the report when rollback failed. In this case the report isn't generated since
    # it would be useless - the system is in undefinded state.
    if returncode == 1 or returncode is None:
        return {}
...
def main():
    ...
    # Set default value, returncode is used in the finally part of the exception handling
    # and if not set, it causes problems if any errors occur prior returncode is set
    returncode = None
    ...
    try:
        ...
        # Gather JSON report
        data = gather_json_report(json_report_file, returncode)
